class UIRefresh {
    ...
    go (){ //05/18/23 not refactoring this....just don't feel like it
		if (game.slots.lines.max >= 1000){
			$("#upgrade").addClass('d-none');
		} 
		$("#upgrade").attr('src', 'img/slots-upgrade-' + (game.slots.lines.max * 10) + ".png")
		if (game.slots.lines.max > 1 && $("#invested").hasClass('d-none')){
			$("#invested").removeClass('d-none')			
		}
		for (let i = 1; i < game.slots.lines.max; i *= 10){
			if ($(".changeLines-" + i).hasClass('d-none')){
				$(".changeLines-" + i).removeClass('d-none');
			}
		}
		...
		if (game.paused){
			$("#pausedPopUp").removeClass('d-none');			
			return;
		}
		$("#pausedPopUp").addClass('d-none');
		if (game.savedAt == null){
			$("#savedAt").html('unsaved!');
		} else {
			
			$("#savedAt").html('saved ' + Math.floor((Date.now() - game.savedAt) / 60000) 
				+ " mins ago! <button id='reset' class='verb btn'>[ reset ] </button>");
		} 
		this.display.go();
		this.fill.go();
		this.images.go();
